...
import com.example.sudokufinal2.dragdrop.DropNumberHandler;
import com.example.sudokufinal2.dragdrop.InputFileDrop;
import com.example.sudokufinal2.level.ChangeLevelListener;
import com.example.sudokufinal2.level.PuzzleFile;
import com.example.sudokufinal2.level.PuzzleLevel;
import com.example.sudokufinal2.listeners.CEValueChangeListener;
import com.example.sudokufinal2.listeners.CheckerButtonListener;
import com.example.sudokufinal2.listeners.RestartButtonListener;
import com.example.sudokufinal2.listeners.Solver;
import com.example.sudokufinal2.undo.Undo;
...
@Theme("sudokufinal2")
public class SudokuFinal2UI extends UI {
...
//	private final HorizontalLayout hLayout = new HorizontalLayout();
	...
	@VaadinServletConfiguration(productionMode = false, ui = SudokuFinal2UI.class)
	...
	protected void init(VaadinRequest request) {
		...
		for (int col = 0; col < 9; col++)
			for (int row = 0; row < 9; row++) {
				...
				wrapper.setDropHandler(new DropNumberHandler(label));
		...
		puzzle = new PuzzleFile(grid, board);
		...
		InputFileDrop dropBox = new InputFileDrop(tab, vlDropPane, puzzle, lbInfoLabel, progress);
...
//		vLayout.setComponentAlignment(undoButton, Alignment.MIDDLE_CENTER);
		 ...
		 */
		undoButton.addClickListener(new Undo());
